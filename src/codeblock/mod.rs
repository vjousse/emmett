...
use crate::config::highlighting::{resolve_syntax_and_theme, HighlightSource};
...
impl<'config> CodeBlock<'config> {
    pub fn new<'fence_info>(
        ...
        let syntax_and_theme = resolve_syntax_and_theme(fence.language, settings);
        ...
        let highlighter = SyntaxHighlighter::new(true, syntax_and_theme);
