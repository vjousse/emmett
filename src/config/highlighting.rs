...
lazy_static! {
    pub static ref SYNTAX_SET: SyntaxSet = {
        let ss: SyntaxSet = from_binary(include_bytes!("../../sublime/syntaxes/newlines.packdump"));
        ss
    };
...
pub fn resolve_syntax_and_theme<'config>(
    ...
    let theme = settings.markdown.get_highlight_theme();

    if let Some(ref lang) = language {
        if let Some(ref extra_syntaxes) = settings.markdown.extra_syntax_set {
